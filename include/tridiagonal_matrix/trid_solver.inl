...
void NaiveSolve(const TridiagonalMatrix<T>& matrix, const std::vector<T>& b, std::vector<T>& x)
{
    ...
    T denom = static_cast<T>(1.0) / (matrix.d_[upper] - matrix.dl_[upper-1] * d_tmp[upper-1]);
    x[upper] = (b[upper] - matrix.dl_[upper-1] * x[upper-1]) * denom;
